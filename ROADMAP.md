...
- 📋 **T327** (deps: T324) **nothing here can say what one conversation cost, only what a window and a project cost** — Every reader aggregates by time window, project or hour, so the session a person remembers working in has no type, no scan and nowhere to be read from. → §LVI
...
- 📋 **T324** (deps: —) **a workflow's agents are drawn as a project of their own, named after the workflow folder** — ProjectOf takes the containing directory, which under subagents/workflows/wf_<id>/ is not the slug: reproduced here as a project called 315, on a colour slot the repo it ran in lost. → §LIV
