...
| `src/Cli/SelfTestCli.cs` | `--selftest [--quick]`: 270+ assertions over the pacing, store, grid, tail, live-rate, series, language-table, method-note, number-format and palette rules, on synthetic inputs, exiting non-zero on failure — run on every push by `.github/workflows/check.yml` and again before an installer is packaged by `build.yml`. Writes only a temp tree and a `selftest` profile dir, both removed. The repo's test suite. |
