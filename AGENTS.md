...
| `src/Profiles/EnvironmentFixture.cs` | The sampled `CLAUDE_CONFIG_DIR` behind `--sample-env` (T231): the disagreements this machine is never in, so T172's mark and T173's read-back are visible without rewriting the developer's registry. Modes resolve from the profiles really here, and sampling is one-way — what makes "writes nothing" true. |
